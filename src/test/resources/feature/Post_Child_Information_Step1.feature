Feature: Register Child API Test
  ...
  Background:
   ...
   Scenario: Register child with valid data
...
Scenario Outline: Validate registration with missing or invalid field values
  Given I prepare the child registration payload with valid data
...
| birthdate      | 2028-01-01      | 400        |Valid birthdate is required.              | childInfo.birthdate       |
...
| zipCode        | 12345           | 400        | Valid US ZIP code is required            | childInfo.zipCode         |